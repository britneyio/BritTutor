...
function wfu_get_filtered_recs($filter) {
	...
	$query = 'SELECT a.*, GROUP_CONCAT(CONCAT_WS("#", b.iduserdata, HEX(b.property), b.propkey, HEX(b.propvalue), b.date_from) ORDER BY b.propkey ASC SEPARATOR "$") AS userdata_raw '.
		...
		if ( $obsolete ) {
			...
			if ( $filerec->uploadid != '' ) {
				...
				foreach ( $items as $item ) {
					if ( trim($item) != "" ) {
						list($id, $label, $key, $value, $date_from) = explode("#", $item);
